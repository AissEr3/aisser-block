...
import static com.aisser.constant.RabbitMQConstant.*;
...
public class RabbitMQConfig {
    ...
    @Bean
    public Queue giteeSpiderQueue() {
        return new Queue(GITEE_SPIDER_QUEUE, true);
    }

    @Bean
    public FanoutExchange giteeSpiderExchange() {
        return new FanoutExchange(GITEE_SPIDER_EXCHANGE, true, false);
    }

    @Bean
    public Binding bindingGiteeSpiderDirect() {
        return BindingBuilder.bind(giteeSpiderQueue()).to(giteeSpiderExchange());
    }
